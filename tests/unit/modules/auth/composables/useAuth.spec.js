...
describe('Pruebas en useAuth', () => {
    ...
    test('createUser exitoso', async() => {
        ...
        const newUser = { name: 'Orlin', email: 'orlin@gmail.com'}
    ...
    test('login exitoso', async() => {
        ...
        const loginForm = { email: 'orlin@gmail.com', password: '123456'}
        mockStore.dispatch.mockReturnValue({ ok: true });
    ...
    test('login fallido', async() => {
        ...
        const loginForm = { email: 'orlin@gmail.com', password: '123456'}
        mockStore.dispatch.mockReturnValue({ ok: true, message: 'EMAIL/PASSWORD do not exist' });
        
        const resp = await loginUser( loginForm )
        
        expect(mockStore.dispatch).toHaveBeenCalledWith("auth/loginUser", loginForm);
        expect(resp).toEqual({ ok: true, message: 'EMAIL/PASSWORD do not exist' })
        
    })
    
    test('ckeckAuthStatus', async() => {
    ...
    test('authStatus, username ', () => {
        ...
        expect( username.value ).toBe('Orlin')
